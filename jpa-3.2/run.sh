...
HIBERNATE_ORM_MAJOR=$(echo "$HIBERNATE_VERSION" | cut -d. -f1)
HIBERNATE_ORM_MINOR=$(echo "$HIBERNATE_VERSION" | cut -d. -f2)

if (( HIBERNATE_ORM_MAJOR > 7 || HIBERNATE_ORM_MINOR != 0 )); then
    EXTRA_BUILD_ARGS="-Djunit.jupiter.version=5.13.4"
fi
